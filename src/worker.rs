...
use crate::model::{Client, ClientStatus, ClientType, WorkUnit, WorkUnitStatus};
use chrono::Utc;
...
impl Worker {
    ...
    pub async fn register(&mut self) -> Result<(), Box<dyn Error>> {
        println!("Registering client: {}", self.client_id);

        let client_data = Client {
            client_id: None, // Server assigns or we send generic request first? Java sends builder without ID initially usually
        ...
        if let Some(id) = registered_client.client_id {
            self.client_id = id;
            println!("Registered with Client ID: {}", self.client_id);
        }
    ...
    pub async fn run(&mut self) {
        ...
        println!("Worker started for client: {}", self.client_id);
    ...
    async fn cycle(&mut self) -> Result<usize, Box<dyn Error>> {
        let work_units = self
            .client
            .get_work_units(&self.client_id, WorkUnitStatus::ASSIGNED, 100)
            .await?;
        ...
        for mut unit in work_units {
            ...
            // Replicate logic: verify if adding edges creates new cliques
            // In the Java logic: getNewCliques(derivedAdjacency...)

            let count = get_new_cliques(graph, self.clique_size, &unit.edges_to_flip);
